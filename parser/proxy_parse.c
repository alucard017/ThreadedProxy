		    ...
		    int buflen)
{
     ...
     /* Parse request line */
     parse->method = __strtok_r(parse->buf, " ", &saveptr);
     ...
     full_addr = __strtok_r(NULL, " ", &saveptr);
     ...
     parse->protocol = __strtok_r(full_addr, "://", &saveptr);
     ...
     parse->host = __strtok_r(NULL, "/", &saveptr);
     if (parse->host == NULL) {
	  debug( "invalid request line, missing host\n");
	  free(tmp_buf);
	  free(parse->buf);
     ...
     parse->path = __strtok_r(NULL, " ", &saveptr);
     ...
     parse->host = __strtok_r(parse->host, ":", &saveptr);
     parse->port = __strtok_r(NULL, "/", &saveptr);
